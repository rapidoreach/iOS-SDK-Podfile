...
public final class RapidoReachConfiguration {
    ...
    /// For local dev (simulator), you can set env `RAPIDOREACH_BASE_URL` or rely on the DEBUG default `http://localhost:8083`.
    public init(baseURL: URL? = nil,
        ...
        if let envBase = ProcessInfo.processInfo.environment["RAPIDOREACH_BASE_URL"],
           let url = URL(string: envBase) {
            ...
            #if DEBUG
            self.baseURL = URL(string: "http://localhost:8083")!
            #else
